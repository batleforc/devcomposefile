...
pub fn convert_to_devfile_with_overrides(
    ...
    for (service_name, service) in &project.services {
        ...
        // When a service has a post_start field, create a Devfile Command from it.
        if !service.post_start.is_empty() {
            ...
            let cmd_id = format!("post-start-{service_name}");
            ...
            rule_traces.push(RuleTrace {
                service: service_name.clone(),
                description: format!("post_start field converted to Devfile command {cmd_id}"),
            });
        }
    }
